#+TITLE: find
#+DESCRIPTION: Org-Man pages - find
...
- -warn, -nowarn :: Turn warning messages on or off. These warnings
  ...
  *-nowarn* otherwise. If a warning message relating to command-line
  ...
  POSIXLY_CORRECT environment variable is set, and *-warn* is also used,
  it is not specified which, if any, warnings will be active.
...
- -name pattern :: Base of file name (the path with the leading
  ...
  match anything (you probably need to use *-path* instead). A warning
  ...
  directory and the files under it, use *-prune* rather than checking
...
** ACTIONS
- -delete :: Delete files; true if removal succeeded. If the removal
  failed, an error message is issued. If *-delete* fails, *find*'s exit
  status will be nonzero (when it eventually exits). Use of *-delete*
...
- -printf format :: True; print /format/ on the standard output,
  ...
  - %Ak :: File's last access time in the format specified by /k/, which
    ...
    24. locale's time representation (H:M:S). The seconds field includes
        a fractional part.
    ...
    2. locale's full month name, variable length (January..December)

    3. locale's date and time (Sat Nov 04 12:02:33 EST 1989). The format
    ...
    21. week number of year with Sunday as first day of week (00..53)

    23. day of week (0..6)

    23. week number of year with Monday as first day of week (00..53)
